...
if ( ! function_exists( 'x_body_class' ) ) :
  function x_body_class( $output ) {
    ...
    $is_blog                          = is_home();
    $blog_style_masonry               = get_theme_mod( 'x_blog_style' ) == 'masonry';
    $post_meta_disabled               = get_theme_mod( 'x_blog_enable_post_meta' ) == 0;

    $is_archive                       = is_archive();
    $archive_style_masonry            = get_theme_mod( 'x_archive_style' ) == 'masonry';
    $is_shop                          = x_is_shop();

    $is_page                          = is_page();
    $page_title_disabled              = get_post_meta( $entry_id, '_x_entry_disable_page_title', true ) == 'on';

    $portfolio_meta_disabled          = get_theme_mod( 'x_portfolio_enable_post_meta' ) == 0;

    $integrity_design_dark            = get_theme_mod( 'x_integrity_design' ) == 'dark';
    $icon_blank_sidebar_active        = $stack == 'icon' && get_post_meta( $entry_id, '_x_icon_blank_template_sidebar', true ) == 'Yes'; 
    $ethos_post_slider_blog_active    = $stack == 'ethos' && is_home() && get_theme_mod( 'x_ethos_post_slider_blog_enable' ) == 1;
    $ethos_post_slider_archive_active = $stack == 'ethos' && ( is_category() || is_tag() ) && get_theme_mod( 'x_ethos_post_slider_archive_enable' ) == 1;
